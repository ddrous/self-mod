...
from selfmod.visualtester import RegTrainer
from ._utils import *
...
class RegVisualTester:
    def __init__(self, trainer, key=None):
        if key is None:
            raise ValueError("Key must be provided for reproducibility.")
        self.key = key

        if not isinstance(trainer, RegTrainer):
            raise ValueError("The trainer object must be of type RegTrainer.")
        self.trainer = trainer
    ...
    def visualizeCelebA(self, 
        ...
        ctx_dims_x = jax.random.randint(ctx_x_key, (3,), 0, self.trainer.learner.context_size)
        ctx_dims_y = jax.random.randint(ctx_y_key, (3,), 0, self.trainer.learner.context_size)
        ...
        fig, ax = plt.subplot_mosaic('ABC;DDD;EFG', figsize=(4*3, 4*3))
        ...
        if dataloader.adaptation == True:  ## Overwrite the above if adaptation
            losses_node = np.vstack(self.trainer.losses_adapt)
            losses_ctx = np.vstack(self.trainer.losses_adapt)
            xis = self.trainer.learner.contexts_adapt.params
        ...
        label_model = "Model Loss" if dataloader.adaptation == False else "Node Loss Adapt"
        ax['C'].plot(losses_node[:,0], label=label_model, color="grey", linewidth=3, alpha=1.0)
        ...
        colors = ['dodgerblue', 'r', 'b', 'g', 'm', 'c', 'y', 'orange', 'purple', 'brown']
        ax['E'].scatter(xis[:,ctx_dims_x[0]], xis[:,ctx_dims_x[1]], s=30, c=colors[0], marker='X')
        ax['F'].scatter(xis[:,ctx_dims_y[1]], xis[:,ctx_dims_y[1]], s=50, c=colors[1], marker='o')
        ax['G'].scatter(xis[:,ctx_dims_x[2]], xis[:,ctx_dims_y[2]], s=50, c=colors[2], marker='+')

        ax['E'].set_title(f'dim {ctx_dims_x[0]} vs dim {ctx_dims_x[1]}')
        ax['E'].set_xlabel(f'dim {ctx_dims_x[0]}')
        ax['E'].set_ylabel(f'dim {ctx_dims_x[1]}')

        ax['F'].set_title(f'dim {ctx_dims_y[0]} vs dim {ctx_dims_y[1]}')
        ax['F'].set_xlabel(f'dim {ctx_dims_y[0]}')
        ax['F'].set_ylabel(f'dim {ctx_dims_y[1]}')

        ax['G'].set_title(f'dim {ctx_dims_x[2]} vs dim {ctx_dims_y[2]}')
        ax['G'].set_xlabel(f'dim {ctx_dims_x[2]}')
        ax['G'].set_ylabel(f'dim {ctx_dims_y[2]}')

        plt.suptitle(f"Results for env={e}", fontsize=14)
