...
if _in_ipython_session:
	...
	args = argparse.Namespace(split='adapt_test', savepath="./tmp_data/", seed=2026, verbose=1)
else:
...
def tanhcos(t):
    return jnp.tanh(jnp.cos(t))

def sine_exp(t):
    return np.sin(t) * np.exp(0.01 * t)
def cos_log(t):
    return np.cos(t) * np.log(t/10 + 1)
# def sin_squared(t):
#     return np.sin(t**2)
# def cosh_sin(t):
#     return np.cosh(np.sin(t))
def sin_linear(t):
    return np.sin(t) + t / 10
def sin_amp_increase(t):
    return np.sin(t) * (1 + 0.02 * t)
...
#   environments = [sinhcos, tanhcos, sine_exp, cos_log, sin_squared, cosh_sin, sin_linear, sin_amp_increase]
  environments = [sinhcos, tanhcos, sine_exp, cos_log, sin_linear, sin_amp_increase]
#   environments = [sin, cos, periodic, expcos, sincos, sinperiodic, sinhperiodic, sinhsin]
...
if _in_ipython_session:
    ...
    for env in range(len(environments)):
        for traj in range(n_traj_per_env):
            if traj==0:
                ax.plot(t_eval, data[env, traj, :, :], label=f'{env} - {environments[env].__name__}', color=colors[env])
            else:
                ax.plot(t_eval, data[env, traj, :, :], color=colors[env])
        # x, v = data[env, 0, :, 0], data[env, 0, :, 1]
        # ax.plot(t_eval,  data[env, 0, :, :], label=f'{env} - {environments[env].__name__}')
