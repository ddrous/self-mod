...
RUN_FOLDDER = './runs/250609-132233-VAE/'
...
class VAE(eqx.Module):
    ...
    def __init__(self, img_size, kernel_size, latent_dim, key):
        ...
        self.encoder = [
            ...
            jax.nn.relu,
            lambda x: x.flatten(),
            eqx.nn.Linear(64*self.img_size[0]*self.img_size[1], 256, key=layer_keys[2]),
            jax.nn.relu,
        ...
        self.decoder = [
            ...
            jax.nn.relu,
            eqx.nn.Linear(256, 64*self.img_size[0]*self.img_size[1], key=layer_keys[5]),
            jax.nn.relu,
            lambda x: x.reshape((64, self.img_size[0], self.img_size[1])),
...
model = VAE(img_size=IMG_SIZE, kernel_size=[3, 3], latent_dim=128, key=model_key)
...
for epoch in range(EPOCHS):
    ...
    for batch_id, batch in enumerate(train_dataloader):
        ...
        # if batch_id < 10:
        print(f"Epoch {epoch}   Batch {batch_id}    Loss: {loss_epoch}", end="\r")

        # if batch_id>100:     ## Just for now !
        #     break
        losses.append(loss)
...
eqx.tree_serialise_leaves(RUN_FOLDDER+"model.pkl", model)
