...
cavia_results = results[(results['method'] == condition['method']) & (results['taylor_order'] == condition['taylor_order']) & (results['context_size'] == condition['context_size'])]
...
for j, param in enumerate(hps[:2]):
    ...
    ## Sweep over that paramter's range
    # for value in hps_range[param]:
    ...
    for other_val1 in hps_range[other_params[0]]:
        for other_val2 in hps_range[other_params[1]]:
            ...
            ## Get the results for CAVIA
            cavia_results = results[(results['method'] == 'CAVIA') & (results[other_params[0]] == other_val1) & (results[other_params[1]] == other_val2)]
            # print(cavia_results)

            ## Get the results for NCF
            ncf_results = results[(results['method'] == 'NCF') & (results[other_params[0]] == other_val1) & (results[other_params[1]] == other_val2)]
            # print(ncf_results)
            ...
            for gu_id, gu in enumerate(gradient_updates):
                ...
                # Plot the confidence interncal with the xticks
                for i, x in enumerate(xticks):
                    label = f'FlashCAVIA-{gu}' if i==0 else None

                    print("\nTHIS\n", cavia_results_gu[ci_metric])
                    if ci_metric == "ci_ind":
                        yerr = cavia_results_gu[ci_metric].iloc[i]*error_mult
                    else:
                        yerr = None
                    ax.errorbar(x, cavia_results_gu[metric].iloc[i], yerr=yerr, marker="o", label=label, color=colors[gu_id], linestyle="-", capsize=4)

                    yi = cavia_results_gu[metric].iloc[i]
                    ci = cavia_results_gu[ci_metric].iloc[i]*error_mult
                    log_yerr_upper = yi+ci
                    log_yerr_lower = np.maximum(yi-ci, 1e-3)
                    log_yerr_lower = np.minimum(log_yerr_lower, 1e-7)
                    yerr = np.array((log_yerr_lower, log_yerr_upper))[:, None]

                    if ci_metric == "ci_ood":
                        yerr = None

                    # all check that results are non-empty
                    if plot_orig and not cavia_results_zg_gu.empty and not maml_results_zg_gu.empty:
                        label = f'CAVIA-{gu}' if i==0 else None
                        ax.errorbar(x, cavia_results_zg_gu[metric].iloc[i], yerr=yerr, marker="s", label=label, color=colors_orig[gu_id], linestyle="--", capsize=4, linewidth=1)

                        label = f'MAML-{gu}' if i==0 else None
                        ax.errorbar(x, maml_results_zg_gu[metric].iloc[i], yerr=yerr, marker="^", label=label, color=colors_orig[gu_id], linestyle="-.", capsize=4, linewidth=1)
...
fig.savefig(f"{save_folder}/TotalLegend.{extension}", bbox_inches='tight')
