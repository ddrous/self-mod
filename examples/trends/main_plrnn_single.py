...
## For the single environment case
train_dataset.dataset = train_dataset.dataset[:1]
train_dataset.t_eval = train_dataset.t_eval[:1]
train_dataset.total_envs = 1
...
class RootRNN(eqx.Module):
    ...
    def __init__(self, data_size, latent_size, hidden_size, key=None):
        ...
        # self.A = xavier_uniform(keys[0], (M, M))
        # self.W1 = xavier_uniform(keys[1], (M, L))
        # self.W2 = xavier_uniform(keys[2], (L, M))
        # self.h2 = xavier_uniform(keys[3], (L,))
        # self.h1 = xavier_uniform(keys[4], (M,))
        ...
        # params = (self.A, self.W1, self.W2, self.h1, self.h2, self.alpha)
    ...
    def __call__(self, xs_gt, params):
        ...
        def f(z, x_gt):
            # z_curr = self.alpha*z + (1-self.alpha)*x_gt     ## Teacher-Forcing
            # z_next = self.A@z_curr + self.W1@jax.nn.relu(self.W2@z_curr + self.h2) + self.h1
