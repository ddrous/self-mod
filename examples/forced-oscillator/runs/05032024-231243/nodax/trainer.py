...
class Trainer:
    ...
    def train_proximal(self, nb_outer_steps, int_prop=1.0, inner_tol=1e-2, nb_inner_steps_max=10, proximal_reg=100., print_error_every=1, save_path=False, val_dataloader=None, val_criterion=None, key=None):
        ...
        @eqx.filter_jit
        def train_step_node(node, node_prev, contexts, batch, weights, opt_state, key):
            print('\nCompiling function "train_step" for neural ode ...')

            def prox_loss_fn(node, contexts, batch, weights, key):
                loss, aux_data = loss_fn(node, contexts, batch, weights, key)
                diff_norm = params_diff_norm_squared(node, node_prev)
        ...
        @eqx.filter_jit
        def train_step_ctx(node, contexts, contexts_prev, batch, weights, opt_state, key):
            print('\nCompiling function "train_step" for context ...')

            def prox_loss_fn(contexts, node, batch, weights, key):
                loss, aux_data = loss_fn(node, contexts, batch, weights, key)
                diff_norm = params_diff_norm_squared(contexts, contexts_prev)
                return loss + proximal_reg * diff_norm / 2., (*aux_data, diff_norm)
        ...
        print(f"    Maximum total number of training steps: {nb_outer_steps*nb_inner_steps_max}")
        ...
        for out_step in range(nb_outer_steps):
            ...
            for in_step_node in range(nb_inner_steps_max):
                ...
                for i, batch in enumerate(self.dataloader):
                    ...
                    node, contexts, opt_state_node, loss_node, (nb_steps_node_, term1, term2, diff_node_) = train_step_node(node, node_prev, contexts, batch, weights, opt_state_node, loss_key)
                ...
                diff_node = params_diff_norm_squared(node, node_prev) / params_norm_squared(node)
                if diff_node < inner_tol:
                    break
            ...
            for in_step_ctx in range(nb_inner_steps_max):
                ...
                for i, batch in enumerate(self.dataloader):
                    ...
                    node, contexts, opt_state_ctx, loss_ctx, (nb_steps_ctx_, term1, term2, diff_ctx_) = train_step_ctx(node, contexts, contexts_prev, batch, weights, opt_state_ctx, loss_key)
                ...
                diff_ctx = params_diff_norm_squared(contexts, contexts_prev) / params_norm_squared(contexts)
                if diff_ctx < inner_tol:
                    break
            ...
            if out_step%print_error_every==0 or out_step<=3 or out_step==out_step-1:
                ...
                print(f"        -NbInnerStepsNode: {in_step_node+1:-3d}\n        -NbInnerStepsContext: {in_step_ctx+1:-3d}\n        -InnerTolerance: {inner_tol:-.9f}\n        -DiffNode: {diff_node:-.9f}\n        -DiffContext: {diff_ctx:-.9f}", flush=True)
